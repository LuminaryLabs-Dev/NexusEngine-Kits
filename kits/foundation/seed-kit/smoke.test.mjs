...
import { createScopedSeed, createSeededRandom } from "nexusengine/foundation";
...
const orderA = createEngine({ kits: [createSeedKit({ seed: "independent" })] }).n.seedStream;
const a1 = orderA.next("a");
const b1 = orderA.next("b");
const a2 = orderA.next("a");
const orderB = createEngine({ kits: [createSeedKit({ seed: "independent" })] }).n.seedStream;
assert.equal(orderB.next("a"), a1);
assert.equal(orderB.next("a"), a2);
assert.equal(orderB.next("b"), b1);

const values = ["a", "b", "c", "d"];
const choiceEngine = createEngine({ kits: [createSeedKit({ seed: "collections" })] }).n.seedStream;
const choiceReplay = createEngine({ kits: [createSeedKit({ seed: "collections" })] }).n.seedStream;
...
const restoredEngine = createEngine({ kits: [createSeedKit({ seed: "other" })] });
...
const limited = createEngine({ kits: [createSeedKit({ maxStreams: 2 })] }).n.seedStream;
...
const scaleEngine = createEngine({ kits: [createSeedKit({ seed: "scale", maxStreams: 1000 })] });
...
const scaleRestore = createEngine({ kits: [createSeedKit({ maxStreams: 1000 })] });
assert.deepEqual(scaleRestore.n.seedStream.loadSnapshot(scaleSnapshot), scaleSnapshot);

const legacy = createEngine({ kits: [createGenericSeedKit(NexusEngine, { seed: "legacy" })] });
...
const installerEngine = createEngine();
...
assert.equal(typeof installerEngine.n.runtime.getSnapshot, "function");
