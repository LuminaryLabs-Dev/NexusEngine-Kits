...
class Peer extends Emitter { static instances = []; constructor(id) { super(); this.id = id ?? `peer-${Peer.instances.length}`; this.open = false; this.connections = []; Peer.instances.push(this); queueMicrotask(() => { this.open = true; this.emit("open", this.id); }); } connect(id, options) { const value = new Connection(id, options); this.connections.push(value); return value; } disconnect() { this.open = false; } destroy() { this.open = false; } }
