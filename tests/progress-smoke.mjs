...
assert.equal(progress.baselineResolved, 2);
assert.equal(progress.baselineRemaining, 118);
assert.equal(progress.official, 4);
assert.equal(progress.candidate, 1);
assert.equal(progress.scaffolded, 9);
assert.equal(progress.placeholder, 108);
...
assert.equal(progress.activeCapability, "seed-kit");
assert.equal(progress.stages.inventoried, 123);
assert.equal(progress.stages.protoValidated, 6);
