...
assert.equal(progress.candidate, 11);
...
assert.equal(progress.approvedAdditionsTotal, 31);
assert.equal(progress.activeCapability, "clock-kit");
assert.equal(progress.stages.inventoried, 151);
assert.equal(progress.stages.protoValidated, 18);
